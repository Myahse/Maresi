...
export function HostApplicationPage() {
  ...
  const approved = isApprovedHost(user) || current?.status === "approved";
  ...
  return (
    <div className="container mx-auto max-w-xl px-4 py-8 space-y-6">
      <div>
        <h1 className="text-2xl font-bold">{t("hostApply.title")}</h1>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border bg-card p-6 space-y-3">
          ...
          <Link to="/owner" className="text-sm font-medium text-primary hover:underline">
            {t("owner.title")}
          </Link>
        </div>
      ) : pending ? (
        ...
        <form onSubmit={handleSubmit} className="rounded-2xl border bg-card p-6 space-y-4">
          ...
          <p className="text-xs text-muted-foreground">{t("hostApply.fixHint")}</p>
